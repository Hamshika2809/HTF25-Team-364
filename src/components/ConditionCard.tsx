...
export default function ConditionCard({ condition, index }: ConditionCardProps) {
  ...
  return (
    ...
    >
      <div
        className={`relative bg-white rounded-2xl shadow-lg border-2 transition-all duration-500 hover:shadow-2xl hover:-translate-y-2 cursor-pointer ${getSeverityBg(
          condition.severity
        ...
        {!isFlipped ? (
          ...
          <div className="p-6">
            ...
            <ul className="space-y-2">
              {condition.remedies.map((remedy, idx) => (
                <li key={idx} className="flex items-start gap-2">
                  <span className={`mt-1.5 w-1.5 h-1.5 rounded-full bg-gradient-to-r ${getSeverityColor(condition.severity)}`} />
            ...
            <div className="mt-6 pt-4 border-t border-gray-200 flex items-center justify-center text-sm text-gray-500">
              ...
              <ChevronUp className="w-4 h-4 ml-1 group-hover:-translate-y-1 transition-transform" />
            </div>
          </div>
        )}
      </div>
